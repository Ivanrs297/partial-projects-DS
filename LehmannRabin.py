...
class Process:
	...
	def rem(self, table):
		print(f"P: {self.i} on remain")
		time.sleep(randrange(5)) # wait randome time
		self.tryin(table)

	def tryin(self, table):
		print(f"P: {self.i} on trying")
		while True:
			# Check if the right fork es available
			if table.forks[self.i % len(table.forks)] == 0:
				table.forks[self.i % len(table.forks)] = 1

				# Check if the left fork es available
				if table.forks[(self.i + 1) % len(table.forks)] == 0:
					table.forks[(self.i + 1) % len(table.forks)] = 1
	...
	def crit(self, table):
		print(f"P: {self.i} on critical... in table {table.forks}")
		time.sleep(4)
		self.exit(table)

	def exit(self, table):
		# Free the forks left and right
		table.forks[self.i % len(table.forks)] = 0
		table.forks[(self.i + 1) % len(table.forks)] = 0
		print(f"P: {self.i} on exit")
		self.rem(table)
...
if __name__ == '__main__':
	...
	eaters = [Process(i) for i in range(n)]  # Create n philosophers
